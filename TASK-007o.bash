#!/bin/bash
# Task 007o: APT コード改行完全保証・UIレイアウト最適化パッチ

WORKDIR="/home/dop/workdir/006_pythonweb"

cd "$WORKDIR" || { echo "エラー: WORKDIRが見つかりません。"; exit 1; }

echo "システム: Task 007o を開始します。仮想環境を有効化します。"
...
cat << 'EOF' > SUMMARY3.md
# プロジェクト: 006_pythonweb (Akutsu Proxy Terminal - APT)
## 現在の動作方針と仕様定義 (SUMMARY3 - Task 007o)

### 1. アーキテクチャ
- **フロントエンド**: `PyQt6` (Python)
  - 行番号表示対応のカスタムエディタ (`QPlainTextEdit`) を採用。
  - 発言者（ユーザ/AI/システム）に応じて行番号の文字色を動的に変更。
  - ブラウザモード/メモ帳モードの入力切替機能をツールバーに左寄せで配置。
- **バックエンド**: `Playwright` (Python/Chromium)
  - JavaScriptによる「DOM一括構築・一括抽出」。画面にマウント済みの実要素からコードを直接退避させるハイブリッド手法により、改行崩れを完全に防ぐ。
  - 遅延ロードによるチャット取得漏れを防ぐ待機ロジックを搭載。

### 2. ディレクトリ構造の役割
- `applog/session/`: Playwrightの永続化セッションのみ保持。
- `tmp1/`: 解析済みテキストのJSONキャッシュデータ。
- `log1/`: バックグラウンドシステムログ。7日経過で自動パージ。

### 3. メニュー機能 (配置順)
- **全キャッシュクリア**: UIをクリアし、`tmp1/` のデータを全削除後、自動で画面履歴取込を実行。
- **削除リロード**: UIと現在のキャッシュを破棄し、Webから強制的に再読み込み。
- **単純リロード**: キャッシュは保持したまま、ページをリロードしてUIを再描画。
- **画面左履歴取込**: リストのクリアと最新50件の履歴取得を実行。
EOF

cat << 'EOF' > task007o_apt_client.py
import sys
import os
import queue
import logging
...
log_filename = os.path.join(LOG_DIR, f"apt_system_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log")
...
class AkutsuProxyTerminal(QWidget):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Akutsu Proxy Terminal (APT) - Task 007o")
        self.resize(900, 850)
...
if __name__ == '__main__':
    ...
    ui = AkutsuProxyTerminal()
    ui.show()
    sys.exit(app.exec())
EOF

python task007o_apt_client.py